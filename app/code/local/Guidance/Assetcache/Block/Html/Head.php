...
class Guidance_Assetcache_Block_Html_Head extends Mage_Page_Block_Html_Head
{
    ...
    protected function &_prepareStaticAndSkinElements($format, array $staticItems, array $skinItems, $mergeCallback = null)
    {
        ...
        foreach ($staticItems as $params => $rows) {
            foreach ($rows as $name) {
                $url = $mergeCallback ? Mage::getBaseDir() . DS . 'js' . DS . $name : $baseJsUrl . $name;
                $filename = Mage::getBaseDir() . DS . 'js' . DS . $name;
                $items[$params][] = $url . '?v=' . Mage::getSingleton('guidance_assetcache/file')->filemtime($filename);
            }
        }
